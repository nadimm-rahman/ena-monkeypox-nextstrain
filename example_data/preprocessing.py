...
def get_args():
    ...
    """
    parser = argparse.ArgumentParser(prog='preprocessing.py', formatter_class=argparse.RawDescriptionHelpFormatter,
                                     epilog="""
        + =========================================================== +
        |  pre_processing.py                                          |
        |  Python script to handle INSDC data and pre-process prior   |
        |  to creation of Nextstrain instance.                        |    
        + =========================================================== +
        """)
    parser.add_argument('-u', '--username', help='Data Hub username (e.g. dcc_XXXXX)', type=str, required=False)
    parser.add_argument('-p', '--password', help='Password for the data hub', type=str, required=False)
    args = parser.parse_args()
    return args


ena_searches = {
    'sequence': {'search_fields': ['country', 'collection_date', 'host', 'strain', 'isolate'], 'query': 'tax_tree(10244) AND country="*" AND collection_date="*"', 'result_type': 'sequence', 'data_portal': 'ena'},
}
format = 'fasta'
...
class process_fastas:
    ...
    def create_record(self, seq, strain, collection_date, country, host):
        new_record = SeqRecord(
            Seq(seq),
            id=str(strain) + "|" + str(self.id) + "|" + str(collection_date) + "|" + str(country) + "|" + str(host)
        )
        return new_record
...
if __name__ == '__main__':
    ...
    processor = process_fastas(ena_results, 'insdc_sequences.fasta', format)            # Instantiate class with information
